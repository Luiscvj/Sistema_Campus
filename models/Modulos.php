...
class Modulos{
    ...
    public function postDataModulo($data){
        $delimiter = ':';
        echo var_dump($data);
    ...
    public function getModulosByTemas($id){
        $sql = "SELECT * FROM modulos WHERE id_tema = :id_tema";
        $stmt = self::$conn->prepare($sql);
    ...
    public function updateModuloById($data,$id){
        $delimiter = ",:";
        $cont = 1;
    ...
    public function delModuloById($id){
        $RESPONSE=[];
